...
@Composable
fun PokemonItem(pokemon: PokemonSpecies, navController: NavHostController) {
//    val backgroundColor = getBackgroundColor(pokemon.types.firstOrNull()?.type?.name)
    ...
    ElevatedCard(
        modifier = Modifier
            ...
            .padding(4.dp)
            .clickable {
                navController.navigate("pokemon_details/${pokemon.name}")
                Log.d("pokemon ID: ", "pokemon_details/${pokemon.name}")
            },
